      ...
      <div class="homework-list-one" v-for="(item, index) in listsArray" :key="index" @click="select(item)">
        ...
        <div class="homework-list-one-img-div">
          ...
          <div class="check-image-container">
            ...
            <div class="check-image" v-for="(item2, index2) in item.beforeCheckImagesView" :key = index2>
        ...
        <div class="homework-list-one-img-div">
          ...
          <div class="check-image" v-for="(item3, index3) in item.afterCheckImagesView" :key = index3>
            <div class="check-image-upload">
...
export default {
  ...
  methods: {
    ...
    goEverday () {
      sessionStorage.setItem('ifConfirmClearImages', true)
      this.$router.push({
    ...
    getTrusteeshipFun () {
      ...
      // console.log(555, self.studentHostingArray)
      if (self.studentHostingArray.length > 0 && self.studentHostingArray[0].feedbackId === self.studentInfo.feedbackId) {
        console.log('从托管进来有缓存:', self.studentHostingArray);
        if (sessionStorage.getItem('ifConfirmClearImages')) {
        //   self.$dialog.confirm({
        //     title: '提示',
        //     message: '是否放弃当前页面数据。'
        //   }).then(() => {
        ...
        //   }).catch(() => {
        //     // on cancel
          self.listsArray = self.studentHostingArray
        //   });
        } else {
          self.listsArray = self.studentHostingArray;
          hostingHttp.getTrusteeship(data).then(r => {
        ...
        sessionStorage.removeItem('ifConfirmClearImages');
        return
      }
      console.log('从托管进来没有缓存', self.studentHostingArray);
      hostingHttp.getTrusteeship(data).then(r => {
        var array = r
